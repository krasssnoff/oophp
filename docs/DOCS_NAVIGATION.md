...
- `static+fluent`: `Arr`, `Str`, `MbStr`, `Date` (via `Date::of(...)` and immutable `DateChain`), `Fs` (path workflow), `Stream` (handle workflow), `Math` (`NumberChain`), `Url` (`UrlChain`)
- `static-first`: `Regex` (with receiver-friendly fluent transforms on `StringChain`)
- `static-only`: `Json`, `Enc`, `Hash`, `Type`, `Net`, `Proc`, `Sys`
